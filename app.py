...
def build_top10_message(df):
    ...
    lines = [
        f"{i+1}. **{row.Ticker}** — {round(row.MoatScore,1)} {row.MoatLabel.split()[0]}"
...
if st.session_state.data is not None:
    ...
    min_score = st.sidebar.slider("Moat Score minimum", 0, 100, 60)
    ...
    filtered["CoreHolding"] = (filtered["MoatScore"] >= 75) & (filtered["MoatTrend"] > 0)
